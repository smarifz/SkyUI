...
var reset, home, movies, music, games;
...
function init() {
	...
	materialArray.push(new THREE.MeshBasicMaterial({
		map : THREE.ImageUtils.loadTexture('images/Home.png')
	}));
	materialArray.push(new THREE.MeshBasicMaterial({
		map : THREE.ImageUtils.loadTexture('images/Music.png')
	}));
	materialArray.push(new THREE.MeshBasicMaterial({
		map : THREE.ImageUtils.loadTexture('images/ComingSoon.png')
	}));
	materialArray.push(new THREE.MeshBasicMaterial({
		map : THREE.ImageUtils.loadTexture('images/Music.png')
	}));
	materialArray.push(new THREE.MeshBasicMaterial({
		map : THREE.ImageUtils.loadTexture('images/Movies.png')
	}));
	materialArray.push(new THREE.MeshBasicMaterial({
		map : THREE.ImageUtils.loadTexture('images/Home.png')
	}));
	...
	loader.load('ThalesAirplane/ThalesAirplane.mtl');
...
function update() {
	...
	reset.addEventListener("mousedown", function() {
		...
		new TWEEN.Tween(camera.up).to({
			...
			y : 1,
			z : 0
		}, 1000).easing(TWEEN.Easing.Back.Out).start();
	...
	movies.addEventListener("mousedown", function() {
		...
		new TWEEN.Tween(camera.position).to({
			...
			y : 15,
			z : 330
		}, 1000).easing(TWEEN.Easing.Cubic.In).start();
	...
	games.addEventListener("mousedown", function() {
		...
		new TWEEN.Tween(camera.position).to({
			...
			y : 20,
			z : 10
		}, 1000).easing(TWEEN.Easing.Circular.InOut).start();
